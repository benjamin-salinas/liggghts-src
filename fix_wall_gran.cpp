  ...
  for (int iCont = 0; iCont < nNeigh ; iCont++, neighborList++)
  {
    ...
    if(deltan <= 0 || deltan < contactDistanceMultiplier*radius[iPart])
    {
      ...
      if(impl){
        ...
        //if (heattransfer_flag()) addRadiation(iPart);
      }
      else
      {
        sidata.r =  r0_ - sidata.deltan;
...
//smuggle xplane
  ...
  void FixWallGran::addRadiation(int ip)
  {
    ...
    double *Temprad = fppa_T->vector_atom;
    double *heatflux = fppa_hf->vector_atom;
    //printf("Temprad: # %.8f\n",Temprad[ip]);  
    ...
    double A_sphere = 4.*3.1415926*radius[ip]*radius[ip];

    double hfr = STEFAN_BOLTZMANN*viewfactor*(tempw-tempi)*A_sphere;
    //printf("radiative heat flux 1: # %.8f\n",hfr);
    //heatflux[i] += hfr;

    if(computeflag_)
  {   
    double hcr = hfr/(Temp_wall-Temprad[ip]);
    heatflux[ip] += hfr;
    Q_add += hfr * update->dt;
    
    if(fppa_htcw)
        fppa_htcw->vector_atom[ip] = hcr;
  }
